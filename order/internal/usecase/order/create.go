...
func (uc *useCase) CreateOrder(ctx context.Context, info usecase.OrderInfo) (string, error) {
	...
	for _, partUUID := range info.PartUUIDs {
		...
		if err != nil {
			return "", fmt.Errorf("part %s not found in inventory: %w", partUUID.String(), err)
		}

		if partInfo.StockQuantity <= 0 {
			return "", fmt.Errorf("part %s is out of stock", partUUID.String())
		}
