...
func streamContainerLogs(ctx context.Context, container testcontainers.Container, out io.Writer) {
	...
	go func() {
		defer func() {
			_ = logs.Close() //nolint:gosec // Ошибка закрытия логов при teardown не критична
		}()

		_, err = io.Copy(out, logs)
		// Игнорируем ошибки EOF и broken pipe — они возникают при нормальном завершении контейнера
		if err != nil && !errors.Is(err, io.EOF) {
			// Не логируем ошибку если это закрытие pipe при teardown
			if errStr := err.Error(); errStr != "read/write on closed pipe" && errStr != "io: read/write on closed pipe" {
