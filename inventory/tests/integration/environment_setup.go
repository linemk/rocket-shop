...
import (
	...
	"github.com/linemk/rocket-shop/platform/pkg/testcontainers"
	...
	"github.com/linemk/rocket-shop/platform/pkg/testcontainers/path"
)
...
const (
	...
	inventoryGRPCHostKey = "INVENTORY_GRPC_HOST"
	inventoryGRPCPortKey = "INVENTORY_GRPC_PORT"
	inventoryMongoURIKey = "INVENTORY_MONGO_URI"
	inventoryMongoDBKey  = "INVENTORY_MONGO_DB"
...
func setupTestEnvironment(ctx context.Context) *TestEnvironment {
	...
	// Формируем MongoDB URI для подключения из контейнера приложения
	mongoURI := fmt.Sprintf("mongodb://%s:%s@%s:27017",
		mongoUsername,
		mongoPassword,
		generatedMongo.Config().ContainerName,
	)

	appEnv := map[string]string{
		// Переопределяем MongoDB URI для подключения к контейнеру MongoDB из testcontainers
		inventoryMongoURIKey: mongoURI,
		inventoryMongoDBKey:  mongoDatabase,
	}
